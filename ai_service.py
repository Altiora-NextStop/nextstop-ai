...
def get_combined_crowd_prediction(crowd_input):
    rule_result = get_rule_based_crowd_prediction(crowd_input)
    ml_result = get_ml_crowd_prediction(crowd_input)

    rule_prediction = rule_result["predicted_crowd_level"]
    ml_prediction = ml_result["predicted_crowd_level"]

    predictions = [rule_prediction, ml_prediction]

    if "High" in predictions:
        final_prediction = "High"
    elif predictions.count("Low") == 2:
    ...
    return {
        ...
        "final_crowd_estimate": final_prediction,
